...
Currently, IPPool is an object representing a set of IPAddress pools to be used
...
`index` to append to the name of the `Metal3DataTemplateName` is random.
For example, let's imagine we have two Metal3Machines: `metal3machine-0` and
`metal3machine-1`, which create the following `metal3datatemplate-0` and
`metal3datatemplate-1` Metal3Data objects, respectively. However, if two nodes
are being upgraded at a time, there is no guarantee that the same indices will be
appended to the respective objects. In fact, they can be in completely reverse
order (i.e., `metal3machine-0` will get `metal3datatemplate-1` and
`metal3machine-1` will get `metal3datatemplate-0`). In order to make it
predictable, we structure IPClaim object name using the BareMetalHost name, as
follows:
...
## How Does the PreAllocations Field Work?
...
We simply add the claim name(s) using the new format (i.e., including the
BareMetalHost name) to the `preAllocations` field in the `IPPool`:
...
spec:
  ...
  prefix: 24
  preAllocations:
    node-0-baremetalv4-pool: 192.168.111.101
    node-1-baremetalv4-pool: 192.168.111.102
```

Since claim names include BareMetalHost names in them, we are able to predict the
