...
Metal3 supports modifying firmware components of the bare metal hosts.
This feature can be used to update the firmware for BIOS or BMC.
The corresponding functionality in Ironic is called
[Firmware Updates](https://docs.openstack.org/ironic/latest/admin/firmware-updates.html).
...
A `HostFirmwareComponents` resource can be created manually or automatically for each host that supports firmware components with the same name and in the same namespace as host.  BareMetal Operator puts the current components information in the `status.components` field:
...
- The `status.components` information is populated with the current values
  detected by Ironic. If an update is executed the newer information will
  be available when the host moves from `available`.
...
enrollment and provisioning (if an update was executed).
...
spec:
  updates:
  - component: bmc
    url: http://192.168.0.1:8080/firmimgFIT.d9
```

This example is for a real hardware and it was manually created:

- The information for `status` and `metadata` will be updated while the host
  is moving to different states.
...
The firmware update for the components are only executed when the host in `preparing` state.
When adding a new `BMH` and manually creating the `HostFirmwareComponents` CRD
for it, you can specify the updates that must occour for that host before it goes to `available`.

In case you have a host that is `provisioned`, and you would like to execute a
firmware update, you need to edit the `HostFirmwareComponents` CRD and the host will be `deprovisioned` so it can go to `preparing` to execute the updates.
