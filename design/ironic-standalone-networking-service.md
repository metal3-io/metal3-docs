...
5. **Share ConfigMap/Secret**: To decouple IrSO from the management of
   switch config files and SSH keys, a ConfigMap and Secret will be shared
   between BMO and IrSO to hold the contents of those files to be passed
   opaquely to the ironic-networking service.
...
BMO reconciles the contents of all `BareMetalSwitch` instance and builds a
config file which is stores into a ConfigMap that is mounted to the
...
config files and re-acting to any changes therein. 
...
type NetworkingService {
	...
	// +optional
	RPCPort *integer `json:"rpcPort,omitempty"`
	...
	// SwitchConfigMapName references a ConfigMap that contains the set of
	// switch config entries to be used by the Ironic Networking service
	SwitchConfigMapName string `json:"switchConfigMapName"`
	...
	// within the SwitchConfigMapName.
...
5. Implement switch configuration ConfigMap and SSH key file Secret generation 
   in the BareMetalSwitch controller
6. Implement Ironic Networking service Pod deployment
7. Implement ironic-image configuration updates for networking support
